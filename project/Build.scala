...
object ScalariformBuild extends Build {
  ...
  lazy val root: Project = Project("root", file("."), settings = commonSettings ++ Seq(
    publish := (),
    publishLocal := ()
  )) aggregate (scalariform, cli, scalariformSbtPlugin, misc)
  ...
  // Main project containing formatter and parser.
  lazy val scalariform: Project = Project("scalariform", file("scalariform"),
    settings = commonSettings ++ sbtbuildinfo.Plugin.buildInfoSettings ++
    Seq(
  ...
  // Commandline tool to perform formatting.
  lazy val cli = Project("cli", file("cli"), settings = commonSettings ++ SbtOneJar.oneJarSettings ++
    Seq(
  ...
  // SBT plugin providing formatting.
  lazy val scalariformSbtPlugin = Project("sbt-plugin", file("sbt-plugin"),
  ...
  lazy val misc: Project = Project("misc", file("misc"), settings = commonSettings ++
    Seq(
